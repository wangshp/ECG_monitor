...
Copyright 2011 Texas Instruments Incorporated. All rights reserved.
  ...
  PROVIDED ???AS IS??กงก่ WITHOUT WARRANTY OF ANY KIND, EITHER EXPRESS OR IMPLIED, 
...
//  EA = 1;           // enable global interrupt  
  ...
  buffer_w[1] = 0x07;         //set 100SPS, 16bits.
  HalI2CWrite(HR_ID, 2, buffer_w, 1);
  ...
  buffer_w[1] = 0xA0;         // enable HR interrupt 
  HalI2CWrite(HR_ID, 2, buffer_w, 1);
...
static void hrMeasNotify(void)
{
  ...
  uint8 *tmp_buf; 
  tmp_buf = &dataBuf_hr[0];
//  for(int i = 0; i < 1; i++)
//  {
  
    buffer_w[0] = FIFO_DATA;      
    HalI2CWrite(HR_ID, 1, buffer_w, 0);
    HalI2CRead(HR_ID, 1, &recv_buf_hr[0]);                                 // Read data   
  ...
  //dataBuf_hr[0] = 1;
    ...
    //1 status(3 BYTES) + 8 channel * 3 BYTES(24bits)=27 B
      ...
      osal_memcpy(&hrMeas.value[0], &dataBuf_hr[0], 1);   //maximum size?
