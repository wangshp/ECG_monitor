...
// For each I2C transaction wait a maximum of 375us (12 ticks on 32kHz sleep
// timer). 20 ticks give 625us which cover most clock stretching cases
#define HAL_I2C_TRANSACTION_TIMEOUT_TICKS_WRITE      12
#define HAL_I2C_TRANSACTION_TIMEOUT_TICKS_READ       20
// We do not generally want to unblock a stop condition. That is why this
// value is high. 2ms gives enough time to service interrupts inbetween.
#define HAL_I2C_TRANSACTION_TIMEOUT_TICKS_STOP       64
...
#define I2C_DISABLE() st( I2CCFG = 0; )
 ...
 * @fn          i2cStop
 *
 * @brief       Issue a STOP condition and wait for confirmation or timeout.
 *
 * input parameters
 *
 * None.
...
{
  uint8 wakeUpAt = ST0 + HAL_I2C_TRANSACTION_TIMEOUT_TICKS_STOP;

  // Must set STO before clearing SI.
  I2CCFG |= I2C_STO;
  I2CCFG &= ~I2C_SI;

  // if timeout requires sleep timer to roll over, wait for rollover first
  if (wakeUpAt < HAL_I2C_TRANSACTION_TIMEOUT_TICKS_STOP)
  {
    while (((I2CCFG & I2C_STO) != 0) && (ST0 > 0));
  }

  // now wait until ST0 reaches timeout value or STO is cleared
  while (((I2CCFG & I2C_STO) != 0) && (ST0 < wakeUpAt));
...
#if HAL_I2C_MASTER
/**************************************************************************************************
 * @fn          i2cMstWaitForTransaction
 *
 * @brief       Waits until a read/write operation either completes or times out.
 *
 * input parameters
 *
 * @param       timeout - how long to wait for transaction to complete in units of
 *                        sleep timer ticks
 *
 * output parameters
 *
 * None.
 *
 * @return      TRUE if transaction was successful, FALSE if transaction timed out.
 */
static bool i2cMstWaitForTransaction( uint8 timeout )
{
  uint8 wakeUpAt = ST0 + timeout;

  // if timeout requires sleep timer to roll over, wait for rollover first
  if (wakeUpAt < timeout)
  {
    while (((I2CCFG & I2C_SI) == 0) && (ST0 > 0));
  }

  // now wait until ST0 reaches timeout value or STO is cleared
  while (((I2CCFG & I2C_SI) == 0) && (ST0 < wakeUpAt));

  // if SI is set, consider transaction successful
  return ((I2CCFG & I2C_SI) != 0);
}

/**************************************************************************************************
 * @fn          i2cMstWriteByte
 *
 * @brief       Attempt to write a single byte on the I2C bus.
 *
 * input parameters
 *
 * @param       value - data byte to be written
 *
 * output parameters
 *
 * None.
 *
 * @return      TRUE if write was successful, FALSE if write timed out.
 */
static bool i2cMstWriteByte( uint8 value )
{
  I2CDATA = value;
  I2CCFG &= ~I2C_SI;
  return ( i2cMstWaitForTransaction( HAL_I2C_TRANSACTION_TIMEOUT_TICKS_WRITE ) );
}

/**************************************************************************************************
 * @fn          i2cMstReadByte
 *
 * @brief       Attempt to read a single byte from the I2C bus.
 *
 * input parameters
 *
 * @param       pBuf - buffer to place data read
 *
 * output parameters
 *
 * None.
 *
 * @return      TRUE if read was successful, FALSE if read timed out.
 */
static bool i2cMstReadByte( uint8 *pBuf )
{
  bool result;

  I2CCFG &= ~I2C_SI;
  result = i2cMstWaitForTransaction( HAL_I2C_TRANSACTION_TIMEOUT_TICKS_READ );
  *pBuf = I2CDATA;
  return ( result );
}
...
static uint8 i2cMstStrt(uint8 address, uint8 RD_WRn)
{
  ...
  {
    address <<= 1;
    (void) i2cMstWriteByte( address | RD_WRn );
  }
...
i2cLen_t HalI2CRead(uint8 address, i2cLen_t len, uint8 *pBuf)
{
  ...
  if (i2cMstStrt(address, 1) != mstAddrAckR)
  {
    len = 0;
  }
  ...
  while (len > 0)
  {
    ...
    if (i2cMstReadByte( pBuf++ ) == FALSE)
    {
      // read timed out, so abort
      break;
    }
...
i2cLen_t HalI2CWrite(uint8 address, i2cLen_t len, uint8 *pBuf, bool stp)
{
  ...
  {
    if (i2cMstWriteByte( *pBuf++ ) == FALSE)
    {
      // write timed out, so abort
      len = cnt;
      break;
    }
