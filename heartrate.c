...
//  HalI2CWrite(HR_ID, 2, buffer_w, 1);
  ...
  */
  ...
  us_delay(500); //need delay after set mode
  buffer_w[0] = INT_STATUS;      //clear power up interrupt
  HalI2CWrite(HR_ID, 1, buffer_w, 0);
  HalI2CRead(HR_ID, 1, &checkpoint1);
   
  buffer_w[0] = INT_EN;
  buffer_w[1] = 0x20;         // enable only HR interrupt 
  HalI2CWrite(HR_ID, 2, buffer_w, 1);
  ...
  buffer_w[1] = 0x43;         //set 50SPS, LED pulse width 1600us
  HalI2CWrite(HR_ID, 2, buffer_w, 1);
  
  buffer_w_2[0] = MODE_CFR;
  HalI2CWrite(HR_ID, 1, buffer_w_2, 0);
  HalI2CRead(HR_ID, 1, &checkpoint1);  

  
  /*read all data in buffer to clear interrupt.
...
*/
  ...
  while(1) 
  {
    ...
    HalI2CWrite(HR_ID, 1, buffer_w_2, 0);
    HalI2CRead(HR_ID, 1, &checkpoint1);
    buffer_w_2[0] = FIFO_W_POINTER;
    HalI2CWrite(HR_ID, 1, buffer_w_2, 0);
    HalI2CRead(HR_ID, 1, &checkpoint1);
    buffer_w_2[0] = FIFO_R_POINTER;
    HalI2CWrite(HR_ID, 1, buffer_w_2, 0);
    HalI2CRead(HR_ID, 1, &checkpoint1);
  }
  /*test 
