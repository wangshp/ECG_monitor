...
uint16 HeartRate_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & HEART_PERIODIC_EVT )  //no matter app want to listen or not, board will send data out.
  {
...
static void heartRateMeasNotify(void)
{
  ...
  if ( heartRateMeas.pValue != NULL )
  {
    ...
    HeartRate_GetParameter(HEARTRATE_COMMAND, &write_value);
    //if(write_value == 0x11)
    //  P1_0 = 1;
    ...
    *p++ = heartRateBpm;
    ...
    if ( HeartRate_MeasNotify( gapConnHandle, &heartRateMeas ) != SUCCESS )  //notify to send data back to app.
    {
...
/*
    systemId[2] = 1;     //DevInfo_SetParameter is related with read operation.--nop
    systemId[1] = 1;    //it actually didn't go to read any new stuff i think.
    systemId[0] = 1;    //because this part code is only called for once as i read many times!
 */
