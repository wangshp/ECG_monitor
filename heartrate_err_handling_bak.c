...
uint8 errBufX[PKT_DATA_SIZE] = {0xFF,0xFF,0xFF,0,0,0,0,0,0,0,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
uint8 errBufY[PKT_DATA_SIZE] = {0xFF,0xFF,0xFF,0,0,0,0,0,0,0,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
uint8 *err_buf;
uint8 err_pkt_state = 0;     // empty packet
uint8 rptr = 0;
uint8 wptr = 0;
...
//    osal_memcpy(&ecgMeas.value, burstData, 12);
  ...
  for(i=0; i<4; i++)
  {
    ...
    if ((err_pkt_state == 1) && (err_buf != NULL))
    {
      osal_memcpy(&ecgMeas.value[2], &err_buf[0], PKT_DATA_SIZE);
      err_pkt_state = 2;
...
//  static uint8 first_time = 1;
  ...
  if (P1IFG & 0x80)
  {
    ...
    if (rptr == NUM_BUF)
    {
      ...
      // read err regr status again only if previous is already sent
//      if (err_pkt_state == 0)     
      {
        // read error registers every NUM_BUF samples
        TI_ADS1293_SPIAutoIncReadReg(TI_ADS1293_ERROR_LOD_REG, &tmp_err_buf[3], 7);
        err_pkt_state = 1;
        err_buf = tmp_err_buf;
      }
//      dataReadyFlag = 1;
