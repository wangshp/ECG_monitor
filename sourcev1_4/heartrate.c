...
Copyright 2011 Texas Instruments Incorporated. All rights reserved.
  ...
  PROVIDED ???AS IS???¡ì?¨¨ WITHOUT WARRANTY OF ANY KIND, EITHER EXPRESS OR IMPLIED, 
...
#define DEFAULT_HR_PERIOD                    100  //NEED better BLE bandwidth
...
//  EA = 1;           // enable global interrupt  
  ...
  buffer_w[1] = 0x03;         //set 50SPS, 16bits.
  HalI2CWrite(HR_ID, 2, buffer_w, 1);
  ...
  buffer_w[1] = 0x20;         // enable HR interrupt 
  HalI2CWrite(HR_ID, 2, buffer_w, 1);
...
//dataReadyFlag = 1;
  ...
  {
    HalI2CInit( i2cClock_267KHZ );
    if (DEFAULT_HR_PERIOD) //start periodic heart rate task, need to be changed
    {
      //ok, it is actually related with notification enable or not!
      osal_start_timerEx( ecg_TaskID, HR_PERIODIC_EVT, DEFAULT_HR_PERIOD );
    }
...
static void hrMeasNotify(void)
{
  ...
  {  
    ...
    osal_memcpy(&hrMeas.value[0], &dataBuf_hr2[0], 16);   //maximum size?
     
    if(hr_MeasNotify( gapConnHandle, &hrMeas) == SUCCESS)
    {
...
static void ecgCB(uint8 event)
{
  if (event == ECG_MEAS_NOTI_ENABLED)
  {
    ...
    if (gapProfileState == GAPROLE_CONNECTED)   //ecg_TaskID, ECG_PERIODIC_EVT,
    {
      osal_start_timerEx( ecg_TaskID, ECG_PERIODIC_EVT, DEFAULT_ECG_PERIOD );
      TI_ADS1293_SPIWriteReg(TI_ADS1293_CONFIG_REG, 
                          TI_ADS1293_CONFIG_REG_VALUE | ADS1293_START_CONV);   // start conversion
...
static void hrCB(uint8 event)
{
  ...
  {
    // stop periodic measurement
    osal_stop_timerEx( ecg_TaskID, HR_PERIODIC_EVT );
   // TI_ADS1293_SPIWriteReg(TI_ADS1293_CONFIG_REG, 
   //                       TI_ADS1293_CONFIG_REG_VALUE & ADS1293_STOP_CONV);    // stop conversion    
    //ecg_running = FALSE;
  }
...
__interrupt void TI_HR_INT(void)
{
//  static uint8 rd_count = 0;
//  static uint8 first_time = 1;
  uint8 *tmp_buf;
 ...
 // I2CCFG = I2CCFG & (~0x08);
  ...
  if (P2IFG & 0x01)
  {
    ...
    //test if it does read data to clear interrupt?
    buffer_w[0] = INT_STATUS;  //0X01
    buffer_w[1] = 0x00;         // enable HR interrupt A0
    HalI2CWrite(HR_ID, 2, buffer_w, 1);
